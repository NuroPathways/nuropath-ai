...
import { Users, FileText, Plus, TrendingUp, MessageCircle, BarChart2, Upload, ShieldAlert } from "lucide-react";
...
export default function ClinicianDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      let allPlans = [];
      if (kids.length > 0) {
        const results = await Promise.all(kids.map(k => base44.entities.BehaviorPlan.filter({ child_id: k.id })));
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto font-inter">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-foreground">{user?.full_name || "Clinician"}</h1>
      </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="outline" className="rounded-xl gap-1.5 text-xs h-8" onClick={() => setShowAddChild(true)}>
              <Plus className="w-3.5 h-3.5" /> Add to Family
            </Button>
