  ...
  useEffect(() => {
    const unsub = auth.onAuthStateChanged(async (u) => {
      if (u) {
        ...
        if (profile.app_role === "parent") navigate("/ParentDashboard");
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4 font-inter">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md text-center">
        ...
        <div className="inline-block px-3 py-1 rounded-full bg-accent/10 text-accent text-xs font-semibold uppercase tracking-wider mb-3">Parent Login</div>
        <h1 className="text-2xl font-semibold text-foreground mb-2">Parent Sign In</h1>
        <p className="text-sm text-muted-foreground mb-8">Access your family dashboard and behavioral guidance</p>
        ...
        <div className="bg-card border border-border rounded-2xl p-8 shadow-sm text-left">
          ...
          <button onClick={() => { setIsRegister(!isRegister); setError(""); }} className="mt-4 w-full text-sm text-muted-foreground hover:text-foreground text-center">
            {isRegister ? "Already have an account? Sign in" : "New parent? Create account"}
          </button>
        </div>
