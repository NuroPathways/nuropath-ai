...
import { ArrowLeft, TrendingUp, BarChart2, AlertTriangle, CheckCircle2, Clock } from "lucide-react";
...
export default function ProgressReports() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setChildren(kids);
      if (kids.length > 0) {
        const logPromises = kids.map(k => base44.entities.BehaviorLog.filter({ child_id: k.id }));
  ...
  const filteredLogs = selectedChildId === "all" ? logs : logs.filter(l => l.child_id === selectedChildId);

  const childName = selectedChildId === "all" ? "All Clients" : children.find(c => c.id === selectedChildId)?.child_name || "";

  // Behavior frequency by type
  const behaviorFreq = {};
  filteredLogs.forEach(l => {
    if (l.behavior_type) {
      behaviorFreq[l.behavior_type] = (behaviorFreq[l.behavior_type] || 0) + 1;
    }
  });
  ...
  return (
    <div className="min-h-screen bg-background font-inter">
      ...
      <div className="p-5 max-w-3xl mx-auto">
        ...
        {loading ? (
          <div className="grid grid-cols-2 gap-3">{[1,2,3,4].map(i => <div key={i} className="h-24 bg-muted rounded-2xl animate-pulse" />)}</div>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-3 mb-6">
              {STAT_CARDS.map((s, i) => (
                <motion.div key={s.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="bg-card border border-border rounded-2xl p-4">
            ...
            {freqData.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-2xl p-5 mb-5">
                <h2 className="font-semibold text-foreground text-sm mb-4">Behavior Frequency — {childName}</h2>
